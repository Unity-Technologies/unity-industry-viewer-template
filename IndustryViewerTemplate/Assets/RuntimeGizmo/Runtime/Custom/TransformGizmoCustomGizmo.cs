...
namespace RuntimeGizmos
{
    ...
    public class TransformGizmoCustomGizmo : MonoBehaviour
    {
        ...
        void Awake()
        {
            if (!transformGizmo && autoFindTransformGizmo)
            {
        ...
        void CheckForSelectedAxis()
        {
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (Physics.Raycast(transformGizmo.myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo, Mathf.Infinity, mask))
                {
                    ...
                    {
                        selectedAxis = customTranslationGizmos.GetSelectedAxis(ShowAxis.All, hitInfo.collider);
                        type = TransformType.Move;
                    }
                    if (selectedAxis == Axis.None && transformGizmo.TransformTypeContains(TransformType.Rotate))
                    {
                        selectedAxis = customRotationGizmos.GetSelectedAxis(transformGizmo.showAxis, hitInfo.collider);
                        type = TransformType.Rotate;
                    }
                    if (selectedAxis == Axis.None && transformGizmo.TransformTypeContains(TransformType.Scale))
                    {
                        selectedAxis = customScaleGizmos.GetSelectedAxis(ShowAxis.All, hitInfo.collider);
    ...
    public class CustomTransformGizmos
    {
        ...
        public Axis GetSelectedAxis(ShowAxis showAxis, Collider selectedCollider)
        {
            if (showAxis is ShowAxis.X or ShowAxis.All && xAxisGizmoCollider != null && xAxisGizmoCollider == selectedCollider)
                return Axis.X;
            if (showAxis is ShowAxis.Y or ShowAxis.All && yAxisGizmoCollider != null && yAxisGizmoCollider == selectedCollider)
                return Axis.Y;
            if (showAxis is ShowAxis.Z or ShowAxis.All && zAxisGizmoCollider != null && zAxisGizmoCollider == selectedCollider)
                return Axis.Z;
            if (showAxis is ShowAxis.All && anyAxisGizmoCollider != null && anyAxisGizmoCollider == selectedCollider)
                return Axis.Any;
